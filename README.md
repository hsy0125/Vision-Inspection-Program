...
## 8. Q&A 대비(짧은 답변 템플릿)
- **Q. NCC와 Blob의 차이는?**  
  - A. NCC는 “모양 기반 템플릿 매칭”, Blob은 “밝기 기반 덩어리 검출”입니다. 이 프로젝트는 모델 유무로 자동 분기합니다.
- **Q. 왜 ROI가 필요한가?**  
  - A. 불필요한 배경을 제외해 오검출을 줄이고, 검사 안정성과 속도를 높이기 위함입니다.
- **Q. 결과 저장은 어디에 되나?**  
  - A. 실행 파일 기준 하위 `Log/` 폴더에 CSV/텍스트 로그로 저장됩니다.
