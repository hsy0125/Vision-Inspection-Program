...
- 결과는 세션에 누적되며 **CSV 저장(자동/수동 가능)**, 화면의 **Inspection Log는 txt로 저장(수동)** 가능하다.
...
- 레시피는 'clear' 버튼으로 초기화 가능

### 2.5 Halcon/NCC 관련 (Halcon 10) 
...
1) **레시피 모델이 있으면**: 이미지 전체에서 패턴을 찾아 **개수 카운트(전역 카운트)**  
2) **레시피 모델이 없으면**: ROI 내부에서 밝기 임계값 기반 **Blob 카운트**
...
- “현재 Halcon 런타임이 없어 실행은 막혀 있지만, 플로우와 로직은 코드상으로 완성되어 있습니다.”
